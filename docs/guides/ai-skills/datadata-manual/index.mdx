---
sidebar_position: 1
title: Datadata Manual
description: datadata-manual skill 和 Datadata MCP Server 的完整说明。
---

# Datadata Manual

`datadata-manual` 是 Datadata 平台的交互式操作手册，通过 **MCP（Model Context Protocol）Server** 直接调用平台能力。AI Agent 可以像调用本地工具一样搜索数据源、执行查询、管理 Data Spaces。

> **生成独立 Python 脚本（爬虫/ETL/批处理）请使用 [`datadata-rest-api`](/docs/guides/ai-skills/datadata-rest-api/) skill。** MCP 专为聊天交互设计，不适用于生成独立运行的脚本文件。
...
| 权限 | 用途 |
| --- | --- |
| `queries:execute-adhoc` | 运行 SQL/DQL 查询 |
